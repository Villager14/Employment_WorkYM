...
{
	float2 uvPostition = uv + float2(2.0f * index / windowSize.x, 0.0f);

	return tex.Sample(samLinear, uvPostition) * (floor(uvPostition.x) * -1.0f + 1.0f);
}
...
float4 MinasuUV(float2 uv, int index)
{
	...
	return tex.Sample(samLinear, uvPostition) * ceil(uvPostition.x);
}
